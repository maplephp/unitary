...
final class TestCase
{
	...
	/**
	 * Check a value against a validation
	 *
	 * @param Closure $value
	...
	public function check(Closure $validation, ?string $describe = null): void
	{
		...
		// Throw error if the value never set
		if (!$this->expect->hasValueBeenSet()) {
			$blunderSoftException = new BlunderSoftException(
				"You must set the value with `against(\"YourValue\")` method before " .
	...
	/**
	 * DEPRECATED: Add a test unit validation using the provided expectation and validation logic
	 *
	 * @param mixed $expect The expected value
	 * @param Closure(Expect, Traverse): bool $validation
	 * @return TestUnit
	 * @throws ErrorException
	 * @deprecated Use `check(Closure)` instead
	 */
