							...
							if (!$unit->isValid()) {
								if ($this->show && $this->case->getHasError()) {
									...
									if ($unit->getStringifyArgs() !== "") {
										...
										$validation = $unit->getValidation();
										if (!empty($validation)) {
											$validationWithPad = $this->padString($test, $validation);
											$this->command->message($this->command->getAnsi()->style(['yellow', 'italic'], $validationWithPad));
										}
									}
